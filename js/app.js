...
function syncAuthorPayoutButton() {
  ...
  const availPhpMinor = getAuthorBalance()?._computed_available_minor ?? (getAuthorBalance()?.available_php_minor || 0);
  const kycOk = !_walletConfigDefaults.author_payout_kyc_required ||
                getAuthorKyc()?.status === 'approved';
  // Tooltip is just a hint — actual blocking happens in the click handler
  if (!getAuthorKyc()?.payment_method)  btn.title = 'Fill in your Payments Info first';
...
// call any time getAuthorKyc() is fresh).
...
document.getElementById('piSaveBtn')?.addEventListener('click', async () => {
  ...
  const qr  = _piUploads.qr  || getAuthorKyc()?.payment_qr_url  || null;
  const id  = _piUploads.id  || getAuthorKyc()?.id_document_url || null;
  const sig = _piUploads.sig || getAuthorKyc()?.signature_url   || null;
