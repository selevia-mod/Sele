...
//   • Read API: getWalletConfig, isUnlocked, resolveUnlockCost
//   • Paywall dialogs: openUnlockDialog, openBulkBookUnlockDialog
//   • Video monet: setupVideoMonetGate, teardownVideoMonetGate
// (wallet.js also exports renderTopbarCoinPill, markUnlocked, getWallet,
// and onWalletChange — all module-internal as far as app.js is
// concerned now. getWallet + onWalletChange were used by the inline
// Store render/subscriber until Stage 13B moved them into store.js.)
...
// confirmDialog (currently unused but reserved for future use).
import {
...
// Stage 13B — Coin Shop + Wallet History modal. Owner module for the
// Store page, HitPay purchase flow, Appwrite migration banner, the
// wallet history modal, the rewarded-ad stub, and the cold-load
// HitPay return handler. Exports actually used here:
//   • initStore — bridge wiring, called from onSignedIn
//   • showStore — exposed because the hash router (#store), the
//     notification routing, and the books.js / videos.js "out of
//     coins" CTAs all route into it. The topbar coin-pill listener
//     moved INTO store.js so app.js no longer wires it.
// store.js auto-boots its own listeners + HitPay-return setTimeout at
// module load — no explicit boot call needed from app.js.
import { initStore, showStore } from './store.js';
// Stage 14 — Goals (daily/weekly/monthly quests + pool claims +
// engagement tick API + panel UI). Owner module for everything quest-
// shaped that was previously inline in app.js. Exports actually used
// here:
//   • Lifecycle: initGoals, resetGoalsState
//   • Engagement API: tickGoal (used by video player), tickGoalUnique
//     (used by share sites + passed to 5 other module bridges)
//   • Convenience wrappers: tickDailyLogin (login tick), tickActiveDay
//     (monthly m_active30 tick — was missing pre-2026-05-17, see
//     commit eae0235)
// All panel UI, RPCs, state, render, and boot wires live INSIDE
// goals.js — app.js doesn't need to import any of those.
import {
  initGoals, resetGoalsState,
  tickGoal, tickGoalUnique,
  tickDailyLogin, tickActiveDay,
} from './goals.js';
...
async function onSignedIn(user) {
  ...
  // Store / Wallet History — Stage 13B. Bridges the 3 UI helpers that
  // store.js can't observe directly (auth getter + hideAllMainPages +
  // setSidebarActive). Critical because the cold-load handleStoreReturn
  // setTimeout fires at module load and polls _bridgesReady — initStore
  // must run AT MOST 5 seconds after import time or the post-payment
  // landing nav silently skips (handler still verifies the purchase row
  // and toasts cancel correctly without the nav).
  initStore({
    getCurrentUser: () => currentUser,
    hideAllMainPages,
    setSidebarActive,
    ...
    // payments. tickGoalUnique is now the goals.js import (Stage 14).
    tickGoalUnique,
  });

  // Goals — Stage 14. Single bridge (getCurrentUser); goals.js wires
  // its own panel boot, listeners, and visibility-refetch at module
  // load. Must run before any tick* call so the bridge is wired when
  // engagement events start firing.
...
window.shareTo = shareTo;
  ...
  try {
    // Goals: tick login + active_day on each daily sign-in. Both are
    // deduped by today's local-time date inside the goals module so a
    // multi-tab open in the same day counts ONCE. tickActiveDay was
    // added 2026-05-17 — the monthly 'Stay active 30 days' counter
    // (m_active30, required-quest gate for the monthly pool) had no
    // caller before that fix, so every user's monthly pool was
    // unclaimable. See goals.js for the dedupe-key format.
    tickDailyLogin();
    tickActiveDay();
  } catch {}
...
async function signOut() {
  ...
  resetWalletState();
  // Goals — Stage 14. Clears _dailyQuestsState in goals.js so the
  // next user signed in on the same browser doesn't see the previous
  // user's progress before initGoals re-loads. The user-scoped
  // localStorage entry (key v3:userId) stays — that's the previous
  // user's cache for their next sign-in.
  resetGoalsState();
...
// Topbar pill click listener moved to js/store.js in Stage 13B —
// store.js wires the click at module load along with the rest of
// the Store/history boot wires.
...
// ── Store page + Wallet History modal ───────────────────────────────────────
// Moved into js/store.js as part of Stage 13B (2026-05-17). The owner
// module holds: showStore, refreshMigrateBanner, renderStoreBalances,
// loadStorePacks, purchasePack, renderStoreAdProgress, playRewardedAd,
// loadWalletHistory + 5 helpers, openWalletHistory, closeWalletHistory,
// handleStoreReturn — plus all module-load boot wires (topbar coin pill
// click, migrate banner buttons, btnWatchAd, history triggers, modal
// close listeners, onWalletChange subscriber, and the cold-load
// HitPay-return setTimeout). App.js imports showStore so the hash
// router (#store) + notification routing + the books.js/videos.js
// "out of coins" CTAs can still call it; the rest is module-internal.
...
// ════════════════════════════════════════════════════════════════════════════
// Moved into js/goals.js as part of Stage 14 (2026-05-17). The owner
// module holds: _dailyQuestsState, QUEST_TAB_META, QUEST_ICONS, all
// period-key helpers, _fetchGoalsFromSupabase, _fireTickGoalRpc,
// _fireClaimGoalPoolRpc, _friendlyGoalClaimError, tickGoal,
// tickGoalUnique, tickDailyLogin, tickActiveDay, _renderQuestsDayStrip,
// renderDailyQuests, wirePoolClaim, _renderQuestsCountdown,
// _scheduleQuestsCountdown, _flyRewardToBalance, toggleDailyQuestsPanel
// — PLUS the boot wires (btnDailyQuests click, .quests-tab clicks,
// outside-click handler, visibility-refetch).
//
// App.js imports tickGoal/tickGoalUnique back for the 7 in-app callers:
// onSignedIn login + active_day ticks (now via tickDailyLogin /
// tickActiveDay wrappers), 2 share sites, video-player minute tick,
// plus 5 cross-module bridge configs that pass tickGoalUnique to
// other init blocks (wallet, store, books, engagement, feed, profile).
